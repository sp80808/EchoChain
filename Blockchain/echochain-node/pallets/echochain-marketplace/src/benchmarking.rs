...
use pallet_echochain::{
	...
	Pallet as EchoChain, Schedule, Script,
};
use pallet_echochain_compute::Pallet as EchoChainCompute;

pub use crate::stub::*;
use crate::Pallet as EchoChainMarketplace;
...
pub fn job_registration_with_reward<T: Config>(
	...
	let r: <T as pallet_echochain::Config>::RegistrationExtra = r.into();
...
where
	T: pallet_balances::Config + pallet_echochain_compute::Config,
{
	...
	if submit {
		let register_call = EchoChainMarketplace::<T>::advertise(
			RawOrigin::Signed(caller.clone()).into(),
			ad.clone(),
		);
		assert_ok!(register_call);
		let _ = EchoChainCompute::<T>::commit(
			&caller,
...
fn setup_pools<T: pallet_echochain_compute::Config>() {
	assert_ok!(EchoChainCompute::<T>::create_pool(
		RawOrigin::Root.into(),
	...
	T: pallet_balances::Config + pallet_echochain_compute::Config,
{
	...
	let register_call = EchoChain::<T>::register_with_min_metrics(
	...
	assert_ok!(register_call);
	let job_id_seq = EchoChain::<T>::job_id_sequence();
	let job_id: JobId<T::AccountId> = (MultiOrigin::EchoChain(caller.clone()), job_id_seq);
	...
	T: pallet_balances::Config,
{
	...
	assert_ok!(EchoChainMarketplace::<T>::deploy(
		RawOrigin::Signed(caller.clone().into()).into(),
		job.clone(),
		pallet_echochain::ScriptMutability::Mutable(Some(caller.clone())),
		None,
		None
	));
	let job_id_seq = EchoChain::<T>::job_id_sequence();
	let job_id = (MultiOrigin::EchoChain(caller.clone()), job_id_seq);
	(caller, job, job_id)
}
	...
	T: pallet_balances::Config,
{
	...
	assert_ok!(
		EchoChainMarketplace::<T>::advertise(RawOrigin::Signed(processor.clone()).into(), ad,)
	);
	...
	assert_ok!(EchoChain::<T>::register(RawOrigin::Signed(consumer.clone()).into(), job.clone()));
	let job_id: JobId<T::AccountId> =
		(MultiOrigin::EchoChain(consumer), EchoChain::<T>::job_id_sequence());
	let status = EchoChainMarketplace::<T>::stored_job_status(&job_id.0, job_id.1);
	...
	<T as pallet_timestamp::Config>::Moment: From<u64>,
{
	...
	assert_ok!(EchoChain::<T>::register(RawOrigin::Signed(consumer.clone()).into(), job.clone()));
	let job_id: JobId<T::AccountId> =
		(MultiOrigin::EchoChain(consumer.clone()), EchoChain::<T>::job_id_sequence());

	pallet_timestamp::Pallet::<T>::set_timestamp((job.schedule.start_time - 120_000).into());

	assert_ok!(EchoChainMarketplace::<T>::propose_execution_matching(
...
where
	T: pallet_balances::Config + pallet_timestamp::Config + pallet_echochain_compute::Config,
	<T as pallet_timestamp::Config>::Moment: From<u64>,
{
	let max_slots = <T as pallet_echochain::Config>::MaxSlots::get() as u8;
	let consumer: T::AccountId =
		consumer.unwrap_or(<T as Config>::BenchmarkHelper::funded_account(0, u32::MAX.into()));
	...
	<T as pallet_timestamp::Config>::Moment: From<u64>,
{
	...
	assert_ok!(EchoChain::<T>::register_with_min_metrics(
		RawOrigin::Signed(consumer.clone()).into(),
		job.clone(),
		vec![(1, 1, 2), (2, 1, 2), (3, 1, 2), (4, 1, 2), (5, 1, 2), (6, 1, 2)]
			.try_into()
	...
	T: pallet_balances::Config,
{
	...
	.try_into()
	.unwrap();
	let call = EchoChainMarketplace::<T>::acknowledge_match(
...
benchmarks! {
	where_clause {  where
		T: pallet_echochain::Config + pallet_balances::Config + pallet_timestamp::Config<Moment = u64> + pallet_echochain_processor_manager::Config + pallet_echochain_compute::Config,
		<T as frame_system::Config>::AccountId: IsType<<<<T as pallet_echochain_processor_manager::Config>::Proof as Verify>::Signer as IdentifyAccount>::AccountId>,
	}
	...
	report {
		...
		let (manager_id, _) = pallet_echochain_processor_manager::Pallet::<T>::do_get_or_create_manager_id(&manager)?;
		pallet_echochain_processor_manager::Pallet::<T>::do_add_processor_manager_pairing(&processor, manager_id)?;
	...
	propose_matching {
		...
		let max_slots = <T as pallet_echochain::Config>::MaxSlots::get();
		setup_pools::<T>();
		for i in 0..x {
	...
	cleanup_assignments {
		...
		let (manager_id, _) = pallet_echochain_processor_manager::Pallet::<T>::do_get_or_create_manager_id(&consumer)?;
		pallet_echochain_processor_manager::Pallet::<T>::do_add_processor_manager_pairing(&processor, manager_id)?;
		let ad = advertisement::<T>(1, 1_000_000);
		assert_ok!(
			EchoChainMarketplace::<T>::advertise(RawOrigin::Signed(processor.clone()).into(), ad)
		...
		for i in 0..x {
			...
			assert_ok!(EchoChain::<T>::register(RawOrigin::Signed(consumer.clone()).into(), job.clone()));
			let job_id_sequence = EchoChain::<T>::job_id_sequence();
			job_ids.push((MultiOrigin::EchoChain(consumer.clone()), job_id_sequence));
			last_job = Some(job);
		}
	...
	deploy {
		...
		assert_ok!(EchoChainMarketplace::<T>::deploy(RawOrigin::Signed(caller.clone()).into(), job, pallet_echochain::ScriptMutability::Mutable(Some(caller)), Some(original_job_id), Some(min_metrics)));
	}

	edit_script {
		let (caller, _, job_id) = deploy_submit_helper::<T>(0, 1);
	}: {
		assert_ok!(EchoChainMarketplace::<T>::edit_script(RawOrigin::Signed(caller.clone()).into(), job_id, script_random_value()));
	}
	...
	transfer_editor {
		...
		assert_ok!(EchoChainMarketplace::<T>::transfer_editor(RawOrigin::Signed(caller.clone()).into(), job_id, Some(new_editor)));
	}

	impl_benchmark_test_suite!(EchoChainMarketplace, mock::ExtBuilder::default().build(), mock::Test);
}
