...
impl<T> From<pallet_echochain::Error<T>> for Error<T> {
	fn from(e: pallet_echochain::Error<T>) -> Self {
		Error::<T>::PalletEchoChain(e)
	}
}

impl<T: Config> From<Error<T>> for pallet_echochain::Error<T> {
...
impl<T> Error<T> {
	...
	pub(crate) fn is_matching_error(&self) -> bool {
		match self {
			...
			Error::PalletEchoChain(e) => matches!(
				*e,
				pallet_echochain::Error::FulfillSourceNotAllowed
					| pallet_echochain::Error::FulfillSourceNotVerified
					| pallet_echochain::Error::AttestationCertificateNotValid
					| pallet_echochain::Error::AttestationUsageExpired
					| pallet_echochain::Error::RevokedCertificate
			),
