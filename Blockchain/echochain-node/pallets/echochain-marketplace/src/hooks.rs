...
use pallet_echochain::{AllowedSourcesUpdate, JobHooks, JobRegistrationFor, StoredJobRegistration};
...
impl<T: Config> JobHooks<T> for Pallet<T> {
	...
	fn register_hook(
		...
		let execution_count = registration.schedule.execution_count();
			...
			requirements.slots as u32 <= <T as pallet_echochain::Config>::MaxSlots::get(),
	...
	/// since this storage point is owned by pallet_echochain.
	fn deregister_hook(job_id: &JobId<T::AccountId>) -> DispatchResultWithPostInfo {
		...
		let registration = <StoredJobRegistration<T>>::get(&job_id.0, job_id.1)
			.ok_or(pallet_echochain::Error::<T>::JobRegistrationNotFound)?;
		...
		match job_status {
			...
			JobStatus::Assigned(_) => {
				...
				let _ = <NextReportIndex<T>>::clear_prefix(
					job_id,
					<T as pallet_echochain::Config>::MaxSlots::get(),
					None,
				);
			},
		}
