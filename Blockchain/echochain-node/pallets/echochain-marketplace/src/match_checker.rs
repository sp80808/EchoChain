...
impl<T: Config> Pallet<T> {
	...
	pub fn finalize_jobs_for(
		...
		for job_id in job_ids {
			...
			pallet_echochain::Pallet::<T>::clear_environment_for(&job_id);
	...
	pub fn now() -> Result<u64, Error<T>> {
		Ok(<T as pallet_echochain::Config>::UnixTime::now()
			.as_millis()
			.try_into()
			.map_err(|_| pallet_echochain::Error::<T>::FailedTimestampConversion)?)
	}
}
