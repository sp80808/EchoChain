...
use pallet_echochain_compute::{ComputeRewardDistributor, EpochOf};
use sp_core::*;
use sp_io;
use sp_std::prelude::*;

use pallet_echochain::{JobModules, ManagerIdProvider, CU32};
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		pallet_balances::GenesisConfig::<Test> {
			balances: vec![
				...
				(pallet_echochain_acount(), INITIAL_BALANCE),
...
frame_support::construct_runtime!(
	pub enum Test {
		...
		EchoChain: pallet_echochain::{Pallet, Call, Storage, Event<T>},
		EchoChainMarketplace: crate::{Pallet, Call, Storage, Event<T>},
		EchoChainCompute: pallet_echochain_compute::{Pallet, Call, Storage, Event<T>},
...
parameter_types! {
	...
	pub const EchoChainPalletId: PalletId = PalletId(*b"acrstpid");
...
impl pallet_echochain::Config for Test {
	type RuntimeEvent = RuntimeEvent;
	...
	type PalletId = EchoChainPalletId;
	...
	type MaxVersions = pallet_echochain::CU32<1>;
	type WeightInfo = pallet_echochain::weights::WeightInfo<Test>;
	#[cfg(feature = "runtime-benchmarks")]
...
impl pallet_echochain_compute::Config for Test {
	type RuntimeEvent = RuntimeEvent;
	type ManagerId = u128;
	type ManagerIdProvider = EchoChainManagerIdProvider;
	type Epoch = Epoch;
...
pub struct EchoChainManagerIdProvider;
impl
	ManagerIdProvider<
		<Test as frame_system::Config>::AccountId,
		<Test as pallet_echochain_compute::Config>::ManagerId,
	> for EchoChainManagerIdProvider
{
	fn create_manager_id(
		id: <Test as pallet_echochain_compute::Config>::ManagerId,
	...
	fn owner_for(
		manager_id: <Test as pallet_echochain_compute::Config>::ManagerId,
...
impl<T: pallet_echochain_compute::Config<I> + mock_pallet::Config<I>, I: 'static>
	...
	T::Balance: From<u64>,
{
	fn calculate_reward(
		ratio: Perquintill,
		epoch: EpochOf<T, I>,
	) -> Result<<T as pallet_echochain_compute::Config<I>>::Balance, DispatchError> {
	...
	fn distribute_reward(
		...
		amount: <T as pallet_echochain_compute::Config<I>>::Balance,
	) -> DispatchResult {
...
pub mod mock_pallet {
	...
	use pallet_echochain_compute::EpochOf;

	#[pallet::config]
	pub trait Config<I: 'static = ()>:
		frame_system::Config + pallet_echochain_compute::Config<I>
	{
...
#[cfg(feature = "runtime-benchmarks")]
impl pallet_echochain::BenchmarkHelper<Test> for TestBenchmarkHelper {
	fn registration_extra(
		_instant_match: bool,
	) -> <Test as pallet_echochain::Config>::RegistrationExtra {
		ExtraFor::<Test> {
...
impl crate::traits::ProcessorInfoProvider<Test> for ProcessorLastSeenProvider {
	fn last_seen(_processor: &<Test as frame_system::Config>::AccountId) -> Option<u128> {
		Some(EchoChainMarketplace::now().unwrap().into())
	}

	fn processor_version(
		_processor: &<Test as frame_system::Config>::AccountId,
	) -> Option<<Test as pallet_echochain::Config>::ProcessorVersion> {
		Some(1)
	}

	fn last_processor_metric(
		processor: &<Test as frame_system::Config>::AccountId,
		pool_id: pallet_echochain::PoolId,
	) -> Option<frame_support::sp_runtime::FixedU128> {
		Some(EchoChainCompute::metrics(processor, pool_id)?.metric)
	}
}

type MaxSlotsFor<T> = <T as pallet_echochain::Config>::MaxSlots;
pub type ProcessorVersionFor<T> = <T as pallet_echochain::Config>::ProcessorVersion;
pub type MaxVersionsFor<T> = <T as pallet_echochain::Config>::MaxVersions;
...
impl Config for Test {
	type RuntimeEvent = RuntimeEvent;
	type MaxAllowedConsumers = pallet_echochain::CU32<4>;
	type Competing = pallet_echochain::CU32<2>;
...
pub fn pallet_echochain_acount() -> <Test as frame_system::Config>::AccountId {
