...
pub type JobRegistrationForMarketplace<T> = JobRegistration<
	...
	<T as pallet_echochain::Config>::MaxAllowedSources,
...
pub type MatchFor<T> =
	Match<<T as frame_system::Config>::AccountId, <T as pallet_echochain::Config>::MaxSlots>;

pub type ExecutionMatchFor<T> =
	ExecutionMatch<<T as frame_system::Config>::AccountId, <T as pallet_echochain::Config>::MaxSlots>;
...
pub const NUMBER_OF_PUB_KEYS: u32 = 5;
...
pub enum PubKey {
	...
	ED25519(PubKeyBytes),
	SECP256r1Encryption(PubKeyBytes),
	SECP256k1Encryption(PubKeyBytes),
}
...
pub type JobAssignmentFor<T> = JobAssignment<
	...
	<T as frame_system::Config>::AccountId,
	<T as pallet_echochain::Config>::MaxAllowedSources,
	<T as pallet_echochain::Config>::RegistrationExtra,
>;
...
///
/// Also used to ensure that EchoChain does not accept more than the expected number of reports (and pays out no more rewards).
#[derive(
	RuntimeDebug,
	Encode,
	Decode,
...
pub type JobRequirementsFor<T> = JobRequirements<
	<T as Config>::Balance,
	<T as frame_system::Config>::AccountId,
	<T as pallet_echochain::Config>::MaxSlots,
	<T as pallet_echochain::Config>::ProcessorVersion,
	<T as pallet_echochain::Config>::MaxVersions,
>;
...
impl RuntimeApiError {
	...
	pub fn log_error(self, e: impl Debug) -> Self {
		log::error!(
			target: "runtime::echochain_marketplace",
			"[{:?}] error: {:?}",
			self,
			e,
		);
