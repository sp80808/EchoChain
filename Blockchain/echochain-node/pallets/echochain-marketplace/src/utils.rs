...
use pallet_echochain::{AllowedSources, MultiOrigin};
...
pub(crate) fn is_consumer_allowed<T: Config>(
	...
	allowed_consumers
		...
		.map(|allowed_consumers| {
			allowed_consumers.iter().any(|allowed_consumer| allowed_consumer == consumer)
		})
		.unwrap_or(true)
}
...
pub fn is_processor_allowed<T: Config>(
	...
	allowed_sources
		.as_ref()
		.map(|allowed_sources| {
			allowed_sources.iter().any(|allowed_source| allowed_source == source)
		})
		.unwrap_or(true)
}
