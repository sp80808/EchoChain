...
class P2PClient:
    ...
    async def _send_local_command(self, command_type, payload):
        try:
            reader, writer = await asyncio.open_connection(self.node_host, self.local_api_port)
            ...
            print(f"Error sending local command to {self.node_host}:{self.local_api_port}: {e}")
            return {'status': 'error', 'message': str(e)}

    async def add_file_and_announce(self, filepath):
        print(f"Client: Requesting to add file {filepath} to P2P system...")
        response = await self._send_local_command('local_add_file', {'filepath': filepath})
        if response.get('status') == 'success':
            file_hash = response.get('file_hash')
            print(f"Client: File added with hash {file_hash}. Announcing content...")
            announce_response = await self._send_local_command('local_announce_content', {'content_hash': file_hash})
            if announce_response.get('status') == 'success':
                print(f"Client: Content {file_hash} announced successfully.")
                return file_hash
            else:
                print(f"Client: Failed to announce content: {announce_response.get('message')}")
                return None
        else:
    ...
    async def discover_content_peers(self, content_hash):
        ...
        response = await self._send_local_command('local_request_content_info', {'content_hash': content_hash})
        if response.get('status') == 'success':
            peers = response.get('peers', [])
            print(f"Client: Discovered peers for {content_hash}: {peers}")
            return peers
        else:
            print(f"Client: Failed to discover content peers: {response.get('message')}")
            return []

    async def request_file_download(self, content_hash):
        print(f"Client: Requesting download for content hash {content_hash}...")
        response = await self._send_local_command('local_request_file', {'content_hash': content_hash})
        if response.get('status') == 'success':
            print(f"Client: File download initiated for {content_hash}. (Check node logs for progress)")
            return True
        else:
            print(f"Client: Failed to initiate file download: {response.get('message')}")
            return False
